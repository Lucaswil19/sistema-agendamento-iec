...
<body class="min-h-dvh bg-brand-50 font-sans text-stone-900 antialiased selection:bg-brand-yellow selection:text-brand-950">
    <main class="login-layout grid min-h-dvh grid-cols-1 grid-rows-[auto_1fr] lg:grid-cols-2 lg:grid-rows-1">
        ...
        <section class="login-panel flex min-w-0 items-center justify-center bg-gradient-to-br from-white via-white to-brand-50 px-5 sm:px-10 lg:px-12 xl:px-16" aria-labelledby="recovery-title">
            <div class="w-full min-w-0 max-w-md">
                <div class="login-heading">
                    <p class="mb-2 text-sm font-semibold text-brand-700">Recuperação de acesso</p>
                    <h2 id="recovery-title" class="text-2xl font-extrabold tracking-tight text-stone-900 sm:text-3xl xl:text-4xl">Esqueci minha senha</h2>
                    <p class="mt-2 text-sm leading-6 text-stone-500 sm:mt-3 sm:text-base">
                        Informe seu e-mail. Se houver uma conta ativa associada, você receberá as instruções para criar uma nova senha.
                    </p>
                </div>
                ...
                @if($errors->any())
                    <x-ui.alert type="error" toast title="Não foi possível enviar a solicitação" :message="$errors->first()" class="mb-6" />
                @endif

                <form action="{{ route('password.email') }}" method="POST" class="login-form grid min-w-0">
                    @csrf
                    ...
                    <div>
                        <md-filled-text-field
                            ...
                            @error('email') aria-invalid="true" error error-text="{{ $message }}" @enderror
                            required
                            autofocus
                        >
                        ...
                        <p id="email-help" class="mt-2 text-xs leading-5 text-stone-500">
                            Por segurança, a resposta será a mesma independentemente de o e-mail estar cadastrado.
                        </p>
                    ...
                    <button type="submit" class="group flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-brand-700 via-brand-600 to-brand-500 px-5 text-sm font-bold text-white shadow-[0_12px_28px_-12px_rgba(240,64,0,0.8)] transition hover:from-brand-800 hover:via-brand-700 hover:to-brand-600 focus:outline-none focus:ring-4 focus:ring-brand-400/30 active:translate-y-px">
                        ...
                        <i data-lucide="send" class="size-5 transition-transform group-hover:translate-x-0.5" aria-hidden="true"></i>
                    </button>
                </form>

                <div class="mt-7 text-center">
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 rounded-lg text-sm font-bold text-brand-700 underline decoration-brand-300 underline-offset-4 transition hover:text-brand-900 focus:outline-none focus:ring-4 focus:ring-brand-400/20">
